...
func (r *repository) DelOneMonthOldStatusMcDetectionLogs() error {
	...
	for {
		...
		_, err = r.elasticsearch.DeleteByQuery([]string{"status_mc_detection_index"}, strings.NewReader(string(queryJSON)))
		if err != nil {
			return err
		}

		log_function.Info("delete old status mc detection from elastic success")
		// sleep for 10 minute
